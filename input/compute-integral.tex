...
\end{flalign*}
\subsubsection{Computation of constraints on $a_{11}$}
...
$v(a_{22})\geq\max\{-(v_2+v_3),-(v_3+v_4)\}\geq-v_3-\min\{v_2,v_4\}$.

Also the constraint \[m_{6,10}=-a_{21}a_{33}\lambda_1\lambda_2\lambda_3\lambda_4\in\mathbb{Z}_p\implies{v_p(a_{21}a_{33}\lambda_1\lambda_2\lambda_3\lambda_4)\geq{0}}\implies\]\[\implies{v_p(a_{21})+v_p(a_{33})}\geq-(v_1+v_2+v_3+v_4),\]
is already implied by the constraints $m_{2,5}$ and $m_{3,7}$.



We also have mutual dependencies between the $a$ elements:
\begin{flalign*}
...
And hence, $v_p(a_{33})\geq-(v_3+v_4)-\min\{0,v_2+v_p(a_{11})\}$.

Now we integrate by $a_{21}$ considering $a_{22}$ as a constant. We have the following constraints:
\begin{enumerate}
...
As seen above, we have the dependency $m_{4,9}=(a_{11}a_{22}-b_{11})\lambda_2\lambda_3\lambda_4\geq{0}\implies{v_p(a_{11})}+v_p(a_{22})\geq$, which implies, after previous computation, that $a_()$




There are more constraints on the $a$ elements, coming from the product elements $m_{5,8},m_{6,8},m_{6,9},m_{7,9},m_{8,10},m_{9,10}$, but they are already implied by the constraints listed above.
\end{document}
